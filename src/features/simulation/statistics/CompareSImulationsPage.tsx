...
export default function CompareSimulationsPage({ firstSimulationId, secondSimulationId }: Props): JSX.Element{
  ...
  const densityChart = renderLoadingErrorOrComponent(
    ...
    barSeriesByEntity={[firstSimulationParsedStats.currentDensityMap, secondSimulationParsedStats.currentDensityMap]}
    lineSeriesByEntity={[firstSimulationParsedStats.totalDensityMap, secondSimulationParsedStats.totalDensityMap]}
    />,
    isFirstSimulationLoading && isSecondSimulationLoading,
    firstSimulationError ?? secondSimulationError,
  );
