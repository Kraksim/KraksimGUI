...
import { FastField, ErrorMessage, FieldArray, FieldProps } from 'formik';
...
export function getGatewaysStatesInitialValues(
  gatewayIds: number[],
): InitialValues {
  return gatewayIds.reduce(
    (reducer, next) => ({
      ...reducer,
      [next]: {
        generators: [
...
function CreateGatewaysStatesForm({
  allowedGateways,
  values,
}: Props): JSX.Element {
            ...
            <FormBox>
              <ControlContainer>
                <FormSelect
                  ...
                  value={currentId}
                  onChange={(e) =>
                    setCurrentId(parseInt(e.target.value as string))
                  }
                >
                  {allowedGateways.map(({ name, id }) => (
                    ...
                    values[currentId].generators.map((generator, index) => (
                      <>
                        ...
                        <ElementBox key={index + 'second'}>
                          ...
                          <Box>
                            <InputLabel
                              htmlFor={`gatewaysStates.${currentId}.generators.${index}.gpsType`}
                            >
                              GPS type
                            </InputLabel>
                            <FastField
                              ...
                              as={FormSelect}
                            >
                              <MenuItem value={'DIJKSTRA_ROAD_LENGTH'}>
                                Dijkstra road length
                              </MenuItem>
                            </FastField>
                            <ErrorMessage
                              name={`gatewaysStates.${currentId}.generators.${index}.gpsType`}
